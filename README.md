...
A small experiment in temporary public conversation. One public square, a permanent `#main` room, and public rooms anyone can create. No accounts, application server, or central message database. Built with TypeScript, vanilla DOM APIs, Vite, and GenosDB **0.34.0**.
...
`#main` is a built-in room and needs no persistent room node. Local identity and chosen display name are saved in `localStorage`; each tab has a fresh presence session ID. Participants are estimated from unexpired presence records, deduplicated by local author ID. Counts include this browser and can lag or be spoofed. The header reports direct GenosRTC application-channel connections, which are not a global census or a message-delivery acknowledgement.
...
Verified locally on 2026-09-08: six validation/store cases and all three production browser tests passed, including two isolated Chromium contexts exchanging real P2P messages and catching up after one page closed. Desktop and 390px mobile layouts were reviewed. An earlier reconnect attempt timed out at 30 seconds; the final run passed, so this remains subject to public discovery availability. GitHub Pages publishing and a test across physical devices on different networks have not been performed.
