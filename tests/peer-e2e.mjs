...
async function health() { return (await fetch(`http://127.0.0.1:${healthPort}/healthz`)).json(); }
let peer;
let peerExited;
async function startPeer() {
  peer = spawn(bun, [resolve(root, 'peer/server.ts')], {
    cwd: state,
    env: { ...process.env, GDB_ROOM: network, GDB_RELAY: publicRelays ? '0' : '1',
      GDB_RELAY_URLS: publicRelays ? externalRelays : '', GDB_DB_PATH: resolve(state, 'chat.sqlite'),
      PORT: String(relayPort), HEALTH_PORT: String(healthPort), HEALTH_HOST: '127.0.0.1', CLEANUP_INTERVAL_MS: '1000' },
  ...
  peerExited = new Promise((resolveExit, reject) => {
    peer.once('exit', resolveExit); peer.once('error', reject);
  });
  await Promise.race([until(async () => (await health()).ok), peerExited.then(() => { throw new Error('Peer exited during startup'); })]);
}
async function stopPeer() {
  ...
  try { await peerExited; } finally { clearTimeout(timer); }
}
let vite;
let browser;
const logs = [];
try {
  await startPeer();
  process.env.VITE_CHAT_NETWORK = network;
  process.env.VITE_CHAT_RELAYS = publicRelays ? externalRelays : `ws://127.0.0.1:${relayPort}`;
  vite = await createServer({ root, server: { host: '127.0.0.1', port: appPort, strictPort: true }, plugins: [{
  ...
  await vite.listen();
  for (const path of ['/peer/server.ts', '/peer/server.ts?raw', '/peer/setup-systemd.sh', '/server-local/server.ts', '/server-local/.env.example']) {
    const response = await fetch(`http://127.0.0.1:${appPort}${path}`);
    assert.equal(response.status, 403, 'private server files must not be served by Vite');
  }
  ...
  await first.page.getByRole('textbox', { name: 'Üzenet', exact: true }).fill('Megmarad a szerver újraindítása után');
  ...
  assert.equal((await health()).records.profiles, 1, 'persistent profile must survive process restart');
  const second = await join(first.words);
  await expect(second.page.locator('.message-text')).toHaveText('Megmarad a szerver újraindítása után', { timeout: 45000 });
  ...
  const value = { kind: 'message', id: crypto.randomUUID(), roomId: 'main', authorId: profile.id, authorName: profile.name, text: 'Expires without browsers', createdAt, expiresAt: createdAt + 30 * 60 * 1000 };
  const signedRecord = signRecord(value, profile, secret);
  await probe.evaluate(async ({ network, relayPort, publicRelays, externalRelays, signedRecord }) => {
    const { gdb } = await import('/vendor/genosdb/index.js');
    const db = await gdb(network, { rtc: { cells: true, ...(publicRelays ? (externalRelays ? {relayUrls: externalRelays.split(',')} : {}) : {relayUrls: [`ws://127.0.0.1:${relayPort}`]}) } });
  ...
  await until(async () => { const h = await health(); return h.records.messages === 1 && h.removed >= 1; }, 30000);
